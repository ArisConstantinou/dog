...
- User correctly reported that the correction masks had replaced the coat with flat colors, changing the white tone and erasing the photographed hair detail. Replaced both flat white and crushed-black outputs with luminance-preserving tone remaps: already-correct white fur is now untouched, while only incorrect dark/tan pixels are lifted and the source texture contrast remains visible; new black patches also retain the underlying fur variation.
- Close visual QA showed the tone remap was still smoother than Leo's real atlas. Replaced the shader-generated colors with projected samples from clean white-fur and black-fur regions of Leo's own bundled photographic texture; existing correct white pixels remain untouched, and both corrected colors now use the same real hair material as the rest of the model.
